...
public class EnemyController : MonoBehaviour
{
    ...
    public GameObject EnemyPivot;       // 적피봇
    public Transform firePoint;     //발사 위치
    public float fireRate = 1f;     //연사 속도
    public float nextFireTime;      //연사 속도 시간 변수
    ...
    void Update()
    {
        ...
        Vector3 targetDirection = (player.position - EnemyPivot.transform.position).normalized; //플레이어와 적 피봇 백터를 뺼셈하여 방향 백터를 설정
        Quaternion targetRotation = Quaternion.LookRotation(targetDirection);     // 회전 값을 정해서 
        EnemyPivot.transform.rotation = Quaternion.Lerp(EnemyPivot.transform.rotation, targetRotation, rotationSpeed * Time.deltaTime);//회전 시켜주눈 함수
        ...
        if (Time.time > nextFireTime)
        {
            ...
            temp.GetComponent<ProjectileMove>().projecttiletype = ProjectileMove.PROJECTILETYPE.ENEMY;
        }
